...
#     x_train, x_test, x_val = np.load(file_path + suffix[0]), np.load(file_path + suffix[1]), np.load(file_path + suffix[2])
#     y_train, y_test, y_val = np.load(file_path + suffix[3]), np.load(file_path + suffix[4]), np.load(file_path + suffix[5])
#
...
#
# print(f"x_train['indices'].shape={x_train['indices'].shape}, x_train['indptr'].shape={x_train['indptr'].shape}")
# for per in x_train.files:
#     print(f"x_train[{per}]={x_train[per]}")
#
# print(type(x_train), x_train.files)
# print(x_train.shape, x_test.shpae, x_val.shape)
...
def find_edges(input, test):
    ...
    tree = HnswIndex(input.shape[1], "L2")
    for index in tqdm(range(input.shape[0])):
        tree.add_data(input[index, :])
    tree.build(n_threads=10)
    # clf = NearestNeighbors(n_neighbors=K + 1, algorithm='ball_tree').fit(input)
    ...
    # _, indices = clf.kneighbors(test)
    print(f"time={time.time()-st_time:.3f}s")

    edge_list = []
    for index1, per in indices:
        for index2 in per:
            if index1 != index2:
...
for dataset in datasets:
    ...
    # x_edge_list = build_x_edges(tra_fea, val_fea, tst_fea)
    ...
    create_json_file(x_edge_list, all_fea, tra_id, val_id, tst_id, dataset, 'X-')
    del tra_val_fea
    del all_fea
    ...
    # y_edge_list = build_y_edges(label_fea)
